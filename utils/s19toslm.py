...
def dump_bytes( filetoprint, addr, data_s):
    for i in xrange(0,4,1):
        filetoprint.write("@%07X\n%s\n" % ( addr+i,  data_s[i*2:(i+1)*2] ))
def dump_bytes_bin(filetoprint, addr, data_s):
    data = bytearray([int(data_s[i:i+2],16) for i in range(0,8,2)])
...
## recalculate the flash offset for Arty A7 100T flash
## 0x00000000-0x003FFFFF: FPGA configuration file
## 0x00400000-end       : data & code
fpga_flash_offset = 0x00400000
fpga_tcdm_off_s = "%08X"%(header_size + fpga_flash_offset)
fpga_l2_off_s = "%08X"%(int(l2_off_s,16) + fpga_flash_offset)

fpga_header_bin = open("fpga_header_" + ("%08X"%(fpga_flash_offset)) + ".bin", "wb")
fpga_inst_bin = open("fpga_inst_" + fpga_l2_off_s + ".bin", "wb")
fpga_data_bin = open("fpga_data_" + fpga_tcdm_off_s + ".bin", "wb")
...
# write out binary file to be used by write_memcfg in Vivado
# FIXME: offset has not been taken into account yet
#        they should be continuous otherwise it will not work!
dump_bytes_bin(fpga_header_bin, 0,  fpga_l2_off_s)
dump_bytes_bin(fpga_header_bin, 4,  l2_start_s)
dump_bytes_bin(fpga_header_bin, 8,  l2_size_s)
dump_bytes_bin(fpga_header_bin, 12, l2_blocks_s)
dump_bytes_bin(fpga_header_bin, 16, fpga_tcdm_off_s)
dump_bytes_bin(fpga_header_bin, 20, tcdm_start_s)
dump_bytes_bin(fpga_header_bin, 24, tcdm_size_s)
dump_bytes_bin(fpga_header_bin, 28, tcdm_blocks_s)
...
for addr in sorted(slm_dict.keys()):
    ...
    if(addr >= l2_start and addr <= l2_end):
        ...
        dump_bytes_bin(fpga_inst_bin, l2_addr*4, data)
    ...
    if(addr >= tcdm_start and addr <= tcdm_end):
        ...
        dump_bytes_bin(fpga_data_bin, tcdm_addr * 4 + header_size, data)
